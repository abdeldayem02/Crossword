...
class CrosswordCreator():
    ...
    def consistent(self, assignment):
        ...
        for var1 in assignment:
            ...
            for neighbor in self.crossword.neighbors(var1):
                overlap=self.crossword.overlaps[var1,neighbor]
                if neighbor not in assignment:
                    continue
                if assignment[var1][overlap[0]]!=assignment[neighbor][overlap[1]]:
                    return False
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        def constraint(var,value):
            """
            For a given variable and value, returns the number of choices
            ruled out for neighbor variables
            """
            constraint=0
    ...
    def select_unassigned_variable(self, assignment):
        ...
        unassigned_var=sorted(unassigned_var,key=lambda neighbors:self.crossword.neighbors(neighbors),reverse=True)
        unassigned_var=sorted(unassigned_var,key=lambda value:len(self.domains[value]))
